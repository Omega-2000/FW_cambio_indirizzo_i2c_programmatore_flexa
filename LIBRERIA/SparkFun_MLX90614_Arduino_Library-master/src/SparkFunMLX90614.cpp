...
//aggiunta io
uint8_t IRTherm::setEEPROM(uint16_t reg, int16_t data)
{
	return writeEEPROM(reg, data);
}

uint8_t IRTherm::setCONFIG(int16_t config)
{
	return writeEEPROM(MLX90614_REGISTER_CONFIG, config);
}

void IRTherm::busRequest(){
	pinMode(SCL, OUTPUT);
...
void IRTherm::printAll()
{
	int16_t conf = 0;
	I2CReadWord(MLX90614_REGISTER_TA, &conf);
	Serial.print("TA - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_TOBJ1, &conf);
	Serial.print("O1 - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_TOBJ2, &conf);
	Serial.print("O2 - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_TOMAX, &conf);
	Serial.print("OMAX - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_TOMIN, &conf);
	Serial.print("OMIN - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_PWMCTRL, &conf);
	Serial.print("PWMCTRL - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_TARANGE, &conf);
	Serial.print("RANGE - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_KE, &conf);
	Serial.print("KE - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_CONFIG, &conf);
	Serial.print("CONF - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_ADDRESS, &conf);
	Serial.print("ADDR  - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_ID0, &conf);
	Serial.print("ID0 - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_ID1, &conf);
	Serial.print("ID1 - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_ID2, &conf);
	Serial.print("ID2 - ");
	Serial.println((uint16_t)conf, HEX);
	I2CReadWord(MLX90614_REGISTER_ID3, &conf);
	Serial.print("ID3  - ");
	Serial.println((uint16_t)conf, HEX);
}
...
{	
	...
	Serial.println("SCRITTURA EFFETTTUATA");
	...
	if (i2cRet == 0) {
		Serial.println("SCRITTURA SUCCESSO");
		return 1;
	} else {
		Serial.println("SCRITTURA FALLITA");
		return 0;
